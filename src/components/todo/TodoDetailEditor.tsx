...
export function TodoDetailEditor({ todoId, currentGroupId, onClose, onRefresh }: TodoDetailEditorProps) {
  ...
  return (
    <div className="fixed inset-0 z-50" onClick={onClose}>
      <div
        className="absolute bg-white rounded-lg shadow-xl border border-gray-200 w-56 p-3 space-y-3"
        onClick={(e) => e.stopPropagation()}
        style={{ top: "50%", left: "50%", transform: "translate(-50%, -50%)" }}
      >
        {/* Group selector */}
        <div>
          <h4 className="text-xs font-semibold text-gray-500 mb-1">分组</h4>
          <div className="space-y-0.5 max-h-32 overflow-y-auto">
            <button
              onClick={() => handleGroupChange(null)}
              className={`w-full text-left text-xs px-2 py-1 rounded ${
                groupId === null ? "bg-blue-50 text-blue-700" : "hover:bg-gray-50"
              }`}
            >
              无分组
            </button>
            {groups.map((g) => (
              <button
                ...
                className={`w-full text-left text-xs px-2 py-1 rounded flex items-center gap-2 ${
                  groupId === g.id ? "bg-blue-50 text-blue-700" : "hover:bg-gray-50"
                }`}
              >
        ...
        {/* Tag selector */}
        <div>
          <h4 className="text-xs font-semibold text-gray-500 mb-1">标签</h4>
          <div className="space-y-0.5 max-h-32 overflow-y-auto">
            {allTags.length === 0 && (
              <p className="text-xs text-gray-400 px-2">暂无标签</p>
            )}
            {allTags.map((tag) => (
              <label
                key={tag.id}
